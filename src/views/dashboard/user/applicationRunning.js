...
function ApplicationRunning() {
    ...
    const appDataOption = localStorage.getItem("app-data-option")
    const userDataID = localStorage.getItem("userDataID")
    const appViewerID = localStorage.getItem("application-viewer-id");

    useEffect(() => {
        if (appDataOption === "create" && (appViewerID === "" || appViewerID === undefined)) {
            axios.post(DEVELOPMENT + '/phases/baseimage_create_from_app', {
                ...
                .then(response => {
                    ...
                    if (response.status === 200 && response.data.response_code === 200) {
                        ...
                        localStorage.setItem("application-viewer-id", response.data.response_url)
        ...
        } else if (appDataOption === "clone" && (appViewerID === "" || appViewerID === undefined)) {
            axios.post(DEVELOPMENT + '/phases/baseimage_clone_from_app', {
                id: "",
                appid: appData,
                newid: appData + "new" + Date.now(),
                env: appData + "env",
                name: appData + "name",
                run: "on",
            }, {
    ...
    return (
        <>
            <Header/>
            <ReactBootstrap.Container fluid={true} id="applicationRunning">
                {
...
export default ApplicationRunning;
